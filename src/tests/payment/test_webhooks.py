...
class TestStripeWebhook:
    ...
    def test_checkout_session_completed_with_valid_signature(
        self, client, checkout_session_completed_payload, user
    ):
    ...
    def test_checkout_session_completed(
        self, client, user, checkout_session_completed_payload
    ):
        """
    ...
    def test_checkout_session_completed_user_does_not_exist(
        self, client, checkout_session_completed_payload
    ):
    ...
    def test_customer_subscription_updated(
        self, customer_subscription_updated_payload, client, user
    ):
        """
        ...
        trial_end = customer_subscription_updated_payload["data"]["object"]["trial_end"]

        expected_trial_end = (
            None
            if trial_end == "null"
            else timezone.make_aware(datetime.fromtimestamp(trial_end))
        )
    ...
    def test_invoice_paid(self, invoice_paid_payload, client, user):
        """
    ...
    def test_customer_subscription_deleted(
        self, customer_subscription_deleted_payload, client, user
    ):
        """
    ...
    def test_unknown_event_type(self, client):
        ...
        sig_header = generate_stripe_signature(unknown_payload, STRIPE_WEBHOOK_SECRET)

        with mock.patch("stripe.Webhook.construct_event") as mock_construct_event:
            mock_construct_event.return_value = unknown_payload
            response = client.post(
                self.url,
